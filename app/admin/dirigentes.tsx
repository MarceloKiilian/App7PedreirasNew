...
import { StyleSheet, Text, View, ScrollView, TouchableOpacity, TextInput, Alert, Modal, FlatList, ActivityIndicator } from 'react-native';
import { Colors } from '../../constants/Colors';
import { UserPlus, Mail, Lock, Shield, ChevronDown, Trash2, ArrowLeft } from 'lucide-react-native';
...
export default function CadastroAdministradores() {
  ...
  const handleDelete = (id: string) => {
    Alert.alert("Confirmar", "Deseja remover este administrador?", [
      ...
      { text: "Remover", style: "destructive", onPress: async () => await deleteDoc(doc(db, "administradores", id)) }
    ]);
  };
  ...
  const renderPerfilItem = ({ item }: { item: typeof PERFIS[0] }) => (
    ...
    >
      <Text style={[styles.pickerItemText, perfil.value === item.value && { color: Colors.primary, fontWeight: 'bold' }]}>
        {item.label}
      </Text>
    </TouchableOpacity>
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={[styles.card, { borderTopWidth: 4, borderTopColor: Colors.accent }]}>
          ...
          <TouchableOpacity style={styles.addButton} onPress={handleAddAdmin} disabled={loading}>
            {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.addButtonText}>Cadastrar Administrador</Text>}
          </TouchableOpacity>
        </View>

        <Text style={styles.sectionTitle}>Administradores Atuais</Text>
        {fetching ? <ActivityIndicator color={Colors.primary} /> : admins.map((item) => (
          <View key={item.id} style={styles.adminItem}>
            ...
            <TouchableOpacity onPress={() => handleDelete(item.id)} style={styles.deleteButton}><Trash2 color="#ff4444" size={20} /></TouchableOpacity>
          </View>
        ))}
      </ScrollView>
      ...
      <Modal visible={showPicker} transparent={true} animationType="fade" onRequestClose={() => setShowPicker(false)}>
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowPicker(false)}>
          <View style={styles.modalContent}>
            ...
            <FlatList data={PERFIS} keyExtractor={(item) => item.value} renderItem={renderPerfilItem} />
          </View>
        </TouchableOpacity>
      </Modal>
    </View>
...
const styles = StyleSheet.create({
  ...
  header: { backgroundColor: Colors.primary, paddingTop: 50, paddingBottom: 20, paddingHorizontal: 20, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderBottomLeftRadius: 20, borderBottomRightRadius: 20 },
  headerTitle: { color: Colors.white, fontSize: 18, fontWeight: 'bold' },
  backButton: { padding: 5 },
  content: { padding: 20 },
  card: { backgroundColor: Colors.white, borderRadius: 15, padding: 20, marginBottom: 30, elevation: 3, borderWidth: 1, borderColor: Colors.border },
  cardHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 20 },
  cardTitle: { fontSize: 18, fontWeight: 'bold', color: Colors.primary, marginLeft: 10 },
  label: { fontSize: 14, color: '#666', marginBottom: 8, fontWeight: '500' },
  inputWrapper: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f5f5f5', borderRadius: 10, paddingHorizontal: 12, marginBottom: 20, borderWidth: 1, borderColor: '#eee', height: 50 },
  inputIcon: { marginRight: 10 },
  input: { flex: 1, fontSize: 16, color: Colors.textDark },
  pickerTrigger: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f5f5f5', borderRadius: 10, paddingHorizontal: 12, marginBottom: 25, borderWidth: 1, borderColor: '#eee', height: 50 },
  pickerTriggerText: { flex: 1, fontSize: 16, color: Colors.textDark },
  addButton: { backgroundColor: Colors.primary, padding: 18, borderRadius: 12, alignItems: 'center', elevation: 2 },
  ...
  adminItem: { backgroundColor: Colors.white, borderRadius: 12, padding: 15, marginBottom: 10, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: Colors.border },
  adminIcon: { backgroundColor: '#fef5e7', padding: 10, borderRadius: 10, marginRight: 15 },
  adminInfo: { flex: 1 },
  ...
  modalContent: { backgroundColor: Colors.white, borderRadius: 20, width: '100%', padding: 20, elevation: 10 },
